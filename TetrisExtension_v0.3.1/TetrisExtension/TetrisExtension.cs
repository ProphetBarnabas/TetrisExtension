...
namespace TetrisExtension
{
    ...
    public class Block
    {
        ...
        public void AddBlockProperties(params BlockProperties[] newProperties)
        {
            List<BlockProperties> prop_lst = Properties.ToList();
            prop_lst.AddRange(newProperties);
        }
    }
    ...
    public class Board
    {
        ...
        public void MoveObjectParts(int newX_pos, int newY_pos)
        {
            ...
            {
                if (blocksToMove[i].XPos + newX_pos >= XSize || blocksToMove[i].YPos + newY_pos >= YSize || blocksToMove[i].XPos + newX_pos < 0 || blocksToMove[i].YPos + newY_pos < 0)
                {
                    canMove = false;
                    break;
                }
                else if (CurrentBoard[blocksToMove[i].XPos + newX_pos, blocksToMove[i].YPos + newY_pos].Value != 0)
                {
                    canMove = false;
                    break;
                }
