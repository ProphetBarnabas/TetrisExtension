...
namespace Object
{
    public class Object
    {
        ...
        public Object(int startLocation, ObjectPart[] objParts, int objPartCount, Action rotationMethod = null)
        {
            RotationMethod = rotationMethod;
            StartLocation = startLocation;
            ObjectPartCount = objPartCount;
            if (objPartCount == objParts.Length)
            {
                ObjectParts = objParts;
            }
            else
            {
                throw new Exception("objPartCount must be equal to objParts.Length");
            }
        }
    }
    ...
    public class BlockBoard
    {
        ...
        public bool RowIsFull()
        {
            for (int i = 0; i < XSize; i++)
            {
                for (int j = 0; j < YSize; j++)
                {
                    if (CurrentBoard[XSize, YSize].Value == 0)
                    {
                        return false;
                    }
                }
        ...
        public void RemoveRow(int index)
        {
            for (int i = 0; i < YSize; i++)
            {
                CurrentBoard[index, i].Value = 0;
                CurrentBoard[index, i].ContainsObjectPart = false;
            }
        }
